...
class _HomePageState extends State<HomePage> {
  Future<List<Course>> _fetchFeaturedCourses(CookieRequest request) async {
    try {
      ...
      if (response['success'] == true) {
        ...
        return courses.take(5).toList(); // Take top 5
      }
    } catch (e) {
  ...
  Future<List<Coach>> _fetchFeaturedCoaches(CookieRequest request) async {
    try {
      final response = await request.get(
      ...
      if (response['success'] == true) {
        ...
        return coaches.take(5).toList(); // Take top 5
      }
    } catch (e) {
  ...
  Future<List<CategoryModel>> _fetchCategories(CookieRequest request) async {
    try {
      final response = await request.get(
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            _buildCategoriesSection(request),
            const SizedBox(height: 32),
            _buildFeaturedCoursesSection(request),
            const SizedBox(height: 32),
            _buildFeaturedCoachesSection(request),
  ...
  Widget _buildCategoriesSection(CookieRequest request) {
    return FutureBuilder<List<CategoryModel>>(
      future: _fetchCategories(request),
  ...
  Widget _buildFeaturedCoursesSection(CookieRequest request) {
    return FutureBuilder<List<Course>>(
      future: _fetchFeaturedCourses(request),
      builder: (context, snapshot) {
  ...
  Widget _buildFeaturedCoachesSection(CookieRequest request) {
    return FutureBuilder<List<Coach>>(
      future: _fetchFeaturedCoaches(request),
      builder: (context, snapshot) {
        if (!snapshot.hasData || snapshot.data!.isEmpty) {
          return const SizedBox.shrink();
        }
